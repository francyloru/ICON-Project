...
def train_and_test(dataset, target_column, localita, anno_test):
    ...
    print(f"   - [RandomForest] Training sugli anni antecedenti al {anno_test} per {localita} (Train size: {len(X_train)})")
    print(f"   - [RandomForest] Test sull'anno {anno_test} (Test size: {len(X_test)})")
    ...
    results_df.to_csv(
        f'dati/parametri/parametri_random_forest_{localita}.csv',
        index=False,
        sep=';',
        decimal=','
    )
    print(f"   - Salvati i parametri in 'parametri_random_forest_{localita}.csv'")
    # -----------------------------------------------------------------------
    ...
    risultati["PRED_RF"] = pred_test
    risultati["PRED_RF"] = risultati["PRED_RF"].round(2)

    risultati.to_csv(
        f'./dati/risultati_dei_modelli/predizioni_random_forest_{localita}.csv',
        index=False,
        sep=';',
        decimal=','
    )
